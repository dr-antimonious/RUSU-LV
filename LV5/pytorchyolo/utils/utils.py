...
def ap_per_class(tp, conf, pred_cls, target_cls):
    ...
    for c in tqdm.tqdm(unique_classes, desc="Computing AP"):
        ...
        if n_p == 0 and n_gt == 0:
            ...
            ##### ODKOMENTIRATI ZA 2. ZADATAK
            #plt.plot(recall_curve, precision_curve)
            #plt.title("Precision-recall curve - class " + str(c))
            #plt.xlabel("Recall")
            #plt.ylabel("Precision")
            #plt.savefig('/content/pr_curves/pr_curve_class_' + str(c) + '.png')
            #plt.figure()
